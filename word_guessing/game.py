...
def play(word):
    ...
    print('Ваше текущее состояние игры: ', display_hangman(tries))
    print('Количество доступных попыток: ', tries)
    print(*word_completion)
    word1 = list(word)
    word1.sort()
    text = []
    word2 = list(word)
    while text != word1 and tries != 0:
        flag = True
        letter = input()
        for i in range(len(letter)):
            if letter[i].upper() not in ru_upper:
                print('Введите букву русского алфавита или слово!')
                flag = False
                break
        if letter.upper() in guessed_letters:
            print('Вы уже вводили эту букву')
        elif letter.upper() not in guessed_letters and len(letter) == 1 and flag:
        ...
        if letter.upper() in guessed_words:
        ...
        elif letter.upper() not in guessed_words and len(letter) > 1 and flag:
            ...
            if letter.upper() == word:
                ...
                print(display_hangman(tries))
        if tries == 0:
            print('Вы исчерпали все попытки. Загаданное слово: ', word)
    if text == word1:
        print('Поздравляем, вы угадали слово! Вы победили!')
...
flag = True
while flag:
    answer = input('Хотите сыграть ещё раз? (д - да, н - нет) ')
    if answer == 'д':
        play(get_word())
    else:
        flag = False